...
export default function Login({
    ...
    return (
        <div className="fixed inset-0 z-100 flex items-center justify-center p-6">
            <div className="absolute inset-0 bg-[#343a40]/60 backdrop-blur-[2px]" onClick={() => !isLoggingIn && setShowLoginModal(false)}></div>
            <div className="relative bg-white w-full max-w-sm rounded shadow-2xl overflow-hidden animate-in zoom-in-95 duration-200 border-t-8 border-[#3f6ad8]">
                    ...
                    <form className="space-y-5" onSubmit={(e) => { e.preventDefault(); handleLogin(); }}>
                        <div>
                            ...
                            <input
                                ...
                                className="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded text-sm focus:ring-2 focus:ring-[#3f6ad8] focus:border-transparent outline-none transition-all font-bold disabled:opacity-50"
                        ...
                        <button 
                            ...
                            disabled={isLoggingIn}
                            type="submit" 
                            className={`w-full bg-[#3f6ad8] hover:bg-[#3458b6] text-white font-black py-4 rounded transition-all shadow-lg shadow-blue-200 uppercase text-xs tracking-[0.2em] mt-4 flex items-center justify-center gap-2 ${isLoggingIn ? 'opacity-70 cursor-wait' : ''}`}
                        >
                            {isLoggingIn ? (
                                <>
                ...
                <div className="bg-slate-50 p-4 text-center border-t border-slate-100">
                    <button 
                        ...
                        className="text-[10px] font-black text-slate-400 hover:text-red-500 uppercase tracking-widest disabled:opacity-30"
                    >
                        Đóng
                    </button>
                </div>
